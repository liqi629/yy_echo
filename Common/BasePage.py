...
class BasePage:
    ...
    def click_element(self, loc, model=None):
        ...
        # finally:
        #     ele.click
        #     logging.info("{0}:元素：{1}第二次点击事件".format(model, loc))
    # 鼠标悬浮
    def hover_element(self, loc, model=None):
        ...
        try:
            ...
            logging.info("{0}:元素：{1}鼠标悬停成功".format(model, loc))
        except:
    ...
    def double_click_element(self, loc, model=None):
        ...
        try:
            ...
            logging.info("{0}:元素：{1}鼠标双击成功".format(model, loc))
        except:
            logging.error("鼠标双击操作失败。")
            self.save_webImgs(model)
            raise
    ...
    def context_click_element(self, loc, model=None):
        ...
        try:
            ...
            logging.info("{0}:元素：{1}鼠标右键点击成功".format(model, loc))
        except:
            logging.error("鼠标右键点击：{0}:元素：{1}操作失败。".format(model, loc))
            self.save_webImgs(model)
            raise
    # 鼠标拖拽
